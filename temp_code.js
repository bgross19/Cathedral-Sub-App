...
/**
 * Fetches the raw staff roster data for the admin settings.
 */
function getStaffRosterForAdmin() {
  try {
    ...
    console.error("Error in getStaffRosterForAdmin: ", err);
    throw new Error("Failed to fetch staff roster: " + err.message);
  }
}

/**
 * Saves (adds or edits) a staff member in the Staff Roster.
 */
function saveStaffMemberAdmin(staffData) {
  ...
  try {
    ...
    var data = sheet.getDataRange().getValues();
    var originalEmail = (staffData.originalEmail || "").toLowerCase().trim();
    var newEmail = staffData.email.toLowerCase().trim();

    var rowIndex = -1;

    if (originalEmail) {
      for (var i = 1; i < data.length; i++) {
        if (String(data[i][1]).toLowerCase().trim() === originalEmail) {
          rowIndex = i + 1;
          break;
        }
      }
    } else {
       // Check if new email already exists to avoid duplicates when adding
       for (var i = 1; i < data.length; i++) {
        if (String(data[i][1]).toLowerCase().trim() === newEmail) {
          rowIndex = i + 1;
          break;
        }
      }
    ...
    if (rowIndex > -1) {
      // Edit existing
      sheet.getRange(rowIndex, 1, 1, 3).setValues([[staffData.name, staffData.email, staffData.duty]]);
    } else {
      // Add new
      sheet.appendRow([staffData.name, staffData.email, staffData.duty]);
    }
    return { success: true };
  } catch (err) {
    console.error("Error in saveStaffMemberAdmin: ", err);
    return { success: false, error: err.message };
  } finally {
    lock.releaseLock();
  }
...
/**
 * Deletes a staff member from the Staff Roster.
 */
function deleteStaffMemberAdmin(email) {
  ...
  try {
    ...
    console.error("Error in deleteStaffMemberAdmin: ", err);
    return { success: false, error: err.message };
  } finally {
    lock.releaseLock();
  }
...
/**
 * Bulk upserts staff roster data from CSV upload.
 */
function bulkUpsertStaffRoster(updates) {
  ...
  try {
    ...
    // Build a map of existing emails to row indexes (1-based for Apps Script)
    var emailRowMap = {};
    for (var i = 1; i < data.length; i++) {
      var rowEmail = String(data[i][1]).toLowerCase().trim();
      if (rowEmail) {
        emailRowMap[rowEmail] = i + 1;
      }
    }
    ...
    for (var j = 0; j < updates.length; j++) {
      ...
      var targetEmail = update.email.toLowerCase().trim();

      if (emailRowMap[targetEmail]) {
        // Update existing (this is less efficient but safe, alternatively we could build a full 2D array and overwrite range)
        // Since sheet updates can be slow in a loop, let's just do it cell by cell if we need to,
        // or for simplicity, set the values.
        sheet.getRange(emailRowMap[targetEmail], 1, 1, 3).setValues([[update.name, update.email, update.duty]]);
        updatedCount++;
      } else {
        // Prepare to append
        rowsToAppend.push([update.name, update.email, update.duty]);
        // Add to map so we don't append duplicates within the same CSV
        emailRowMap[targetEmail] = -1;
        updatedCount++;
      }
    }
    ...
    console.error("Error in bulkUpsertStaffRoster: ", err);
    return { success: false, error: err.message };
  } finally {
    lock.releaseLock();
  }
