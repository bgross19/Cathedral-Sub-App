...
let mockSheet = {
  ...
  getRange: (row, col) => {
    return {
      getValue: () => {
        // simulate API delay (Apps Script API calls take around 10-50ms typically)
...
function optimized() {
  ...
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][0]) === String(absenceId)) {
      var subColumnIndex = 10 + parseInt(period) - 1;
      // In JS array is 0-indexed, but getValues returns a 2D array where col 1 is index 0.
...
console.log(`Original Time: ${(t1 - t0).toFixed(2)} ms`);

console.log("Running optimized...");
let t2 = performance.now();
let res2 = optimized();
let t3 = performance.now();
console.log(`Optimized Time: ${(t3 - t2).toFixed(2)} ms`);
