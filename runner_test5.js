...
// The code.gs contains `function abc()` which node's eval will evaluate but won't attach to the global scope
// if it's running in strict mode or a block.
// However, executing code in a new context will attach them to that context.
const vm = require('vm');
...
let roster = context.getStaffRosterForAdmin();
...
context.saveStaffMemberAdmin({name: 'Charlie', email: 'charlie@example.com', duty: '3'});
roster = context.getStaffRosterForAdmin();
...
context.saveStaffMemberAdmin({originalEmail: 'alice@example.com', name: 'Alice M', email: 'alice.m@example.com', duty: '1B'});
roster = context.getStaffRosterForAdmin();
...
context.deleteStaffMemberAdmin('bob@example.com');
roster = context.getStaffRosterForAdmin();
